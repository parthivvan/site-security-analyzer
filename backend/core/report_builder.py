"""Shared scan report shaping for API responses and saved scan results."""
...
def compute_score_from_flat_report(flat_report: Dict[str, bool]) -> int:
    """Single source of truth for score so the number matches the displayed checks."""
    score = 0
...
def build_priority_actions(flat_report: Dict[str, bool]) -> List[Dict[str, str]]:
    """Return the most important next fixes in judge/interviewer-friendly language."""
    actions = []
    ...
    actions.sort(key=lambda action: SEVERITY_ORDER[action["severity"]])
    return actions[:5]
...
def build_risk_snapshot(flat_report: Dict[str, bool], score: int) -> Dict[str, Any]:
    ...
    return {
        ...
        "quick_win_count": sum(1 for action in actions if action["effort"] == "Low"),
    }


def build_explanation(flat_report: Dict[str, bool], score: int) -> str:
    passed = [LABELS.get(key, key) for key, value in flat_report.items() if key != "server_header" and value]
    failed = [LABELS.get(key, key) for key, value in flat_report.items() if key != "server_header" and not value]
    explanation = (
...
def enrich_scan_result(result: Dict[str, Any], flat_report: Dict[str, bool], score: int) -> Dict[str, Any]:
